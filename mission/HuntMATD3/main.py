...
from MultiTargetEnv import MultiTarEnv
...
def trainOrTest(args, bTrain = False):
    ...
    model_dir = os.path.join(os.getcwd(), "models", timestamp)
    if bTrain:
    ...
    bAddNoise = True if bTrain else False
    for episode in range(1, args.num_episodes + 1):
        ...
        while (not done) and (current_step <= args.max_steps):
            ...
            for i, hunter in enumerate(hunters):
                action = hunter.select_action(h_obs[i], noise = bAddNoise)
                actions_hunters.append(action)

            # targets choose action
            for i, target in enumerate(targets):
                action = target.select_action(t_obs[i], noise = bAddNoise)
        ...
        if bTrain:
            ...
            if should_save:
                save_dir = os.path.join(model_dir, f"{save_reason}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ifrender', type=bool, default=0, help='whether to render the environment')
    parser.add_argument('--visualizelaser', type=bool, default=0, help='whether to visualize laser')
    ...
    parser.add_argument('--iforthogonalize', type=bool, default=1, help='whether to orthogonalize the weights')
    parser.add_argument('--iflrdecay', type=bool, default=0, help='whether to decay the learning rate')
    ...
    parser.add_argument('--checkpoint', type=str, default="models/20250721_150758/score", help='path to checkpoint directory for continue training')

    args = parser.parse_args()
    
    trainOrTest(args, bTrain = False)
